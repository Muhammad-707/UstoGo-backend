...
**Enforcement:** `AUTHORIZATION.md` describes the guard implementation. This document is the normative source of *what* each role may do.
...
**Creation:** never via HTTP. Seeded with `npm run cli -- admin:create --email=... --password=...`. The registration controller has no admin branch.
...
*Authentication* — identical to CLIENT.
...
*Messaging* — mirror of CLIENT.

*Notifications* — mirror of CLIENT.
