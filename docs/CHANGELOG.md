...
- **CI pipeline and end-to-end test harness** (closes §1.10). `test/setup/global-setup.ts` starts real PostgreSQL and MinIO containers via Testcontainers, runs `prisma migrate deploy` and the seed against them, and boots the unmodified `AppModule` — the e2e suites exercise the real request pipeline, not a stubbed one. `test-app.factory.ts` overrides only `ThrottlerStorage` (never the guard itself — it is registered under `APP_GUARD`, so `overrideGuard` has no provider to replace) so a suite not about rate limiting is not incidentally asserting it. `auth.helper.ts` and `authz-matrix.helper.ts` generate the six-case authorization matrix — 401 no token, 401 malformed, 401 forged signature, 403 wrong role, 404 (not 403) for a foreign resource, 200 without over-exposed fields — from one spec per protected endpoint, so the case that is usually forgotten cannot be forgotten selectively. Four suites land on it: `auth`, `users`, `files`, `throttling` — 54 tests.
