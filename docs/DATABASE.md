...
A Prisma client extension rewrites every `find*` and `count` on soft-deletable models to append `deletedAt: null`. Admin queries that need deleted rows call the repository's explicit `withDeleted()` variant. This is centralised in `src/prisma/extensions/soft-delete.extension.ts` — no service filters `deletedAt` by hand.
