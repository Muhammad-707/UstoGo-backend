...
| PATCH | `/users/me/avatar` | Any | Attach a confirmed `fileKey` |
...
| Method | Path | Auth | Description |
| --- | --- | --- | --- |
| POST | `/files/presign` | Any | `{ purpose, mimeType, sizeBytes }` → `{ fileId, uploadUrl, fileKey, expiresIn }` |
| POST | `/files/:id/confirm` | Any | Server verifies the object, marks it confirmed |

Constraints: avatars `image/jpeg|png|webp` ≤ 5 MB; certificates additionally `application/pdf` ≤ 10 MB; attachments ≤ 10 MB. Rate limit 20 presigns/hour/user.
...
```json
{
  ...
  "address": { "line": "12 Mustaqillik str, apt 4", "district": "Yunusabad", "latitude": 41.3, "longitude": 69.28 },
...
```json
{
  ...
  "bookings": { "pending": 33, "accepted": 51, "inProgress": 4, "completed": 892, "cancelled": 61, "expired": 27 },
