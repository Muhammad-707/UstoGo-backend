...
A required nested object needs a third decorator, `@IsDefined()`, as in the example above. `@ValidateNested()` runs the child's decorators only when the value exists, so omitting the property entirely passes validation — the same silent hole as forgetting `@Type()`, and harder to spot because the child's rules look thorough. Pinned by a regression test in `validation-pipe.spec.ts`.
...
```json
{
  ...
  "details": [
    ...
    {
      "field": "password",
      "constraints": ["password must be longer than or equal to 8 characters"]
    },
