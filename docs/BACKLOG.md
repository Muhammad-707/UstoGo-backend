...
| B-77 | Redis-backed, identifier-scoped rate limiting              | P1       | S    | `AUTHENTICATION.md` §9 keys the login limit on IP+email, forgot-password on email and refresh on userId; v1 keys everything on IP, so limits are coarser than documented and per-instance rather than global. Blocked on open decision D-5 (managed vs self-hosted Redis).                                                                                                                                                       |
