...
export const truncateAll = async (prisma: PrismaService): Promise<void> => {
  ...
  await prisma.$executeRaw`TRUNCATE TABLE "users", "client_profiles", "master_profiles", "refresh_tokens", "password_reset_tokens", "email_verification_tokens", "two_factor_challenges", "idempotency_keys", "files", "audit_logs", "categories", "master_categories", "certificates", "services", "bookings", "booking_status_history", "reviews", "review_replies", "notifications", "conversations", "messages", "message_attachments", "banners", "product_categories", "products", "product_images", "product_likes", "cart_items", "orders", "order_items" RESTART IDENTITY CASCADE`;
};
